...
namespace CSP_MapColoring
{
    class Heuristic
    {
        ...
        public static Dictionary<int, List<int>> Graph { get; set; }
        ...
        public static void Color(IEnumerable<ArrayList> colors)
        {
        ...
        {
            int Notcolored=0;// = ColoredMap.Where(c => c == 0).Count();
            
            foreach (var item in ColoredMap)
                if (item == null) Notcolored++;
            ...
            for (int item = 1; item < ColoredMap.Count; item++)
        ...
        /// <param name="colors"></param>
        private static void DFSsearch(int vertex, IEnumerable<ArrayList> colors)
        {
            _Visited.Add(vertex);
        ...
        /// <param name="colors"></param>
        /// <returns></returns>
        private static int GetLCV(int vertex, IEnumerable<ArrayList> colors)
        {
            Dictionary<int, object> lcvs = new Dictionary<int, object>();
            ...
            foreach (var color in colors)
            {
                ...
                if (isSafeColor)
                    lcvs.Add(color.IndexOf(color), total);
            }
